...
System.register(
  "virtual-node-diff",
  ["compute-attributes-updates", "extract-attributes"],
        ...
        exports_7(
          ...
          diff = ({ original, updated, parentVirtualNode }) => {
            if (
              ...
              // Compare attributes
              const originalAttrs = extract_attributes_ts_1.extractAttributes(
                original,
              );
              const updatedAttrs = extract_attributes_ts_1.extractAttributes(
                updated,
              );
...
System.register(
  "mount",
  ["extract-attributes", "set-event-handler"],
          ...
          mount = ({ virtualNode }) => {
            ...
            Object.keys(virtualNode.events ?? {})
              .map((eventName) => {
                ...
                set_event_handler_ts_1.setEventHandler(node, eventName, action);
              });
            const attributes = extract_attributes_ts_2.extractAttributes(
              virtualNode,
            );
        ...
        exports_10(
          ...
          applyPatches = ({ patches, mountedVirtualNode }) => {
            return patches.reduce((updatedMountedVirtualNode, patch) => {
              switch (patch.type) {
                case "add_node": {
                  const { virtualNode, node } = mount_ts_1.mount(
                ...
                case "remove_node": {
                  ...
                  const index = patch.originalNode.children
                    ?.findIndex((child) =>
                      child.ref === patch.nodeToBeRemoved
                    ) ?? 0;
                  if (index > -1) {
                ...
                case "update_style_attribute": {
                  patch.originalNode.ref.style[patch.attributeName] =
                    patch.value;
                ...
                case "update_event_attribute": {
                  if (patch.action) {
                    ...
                    patch.originalNode.events[patch.attributeName] =
                      patch.action;
                  }
